        ...
        if(dist[nodeIndex]!=-1){
            ...
            if(!adjacentEdges.empty()){
                for(auto ele: adjacentEdges){
                    ...
                    if (dist[ele] == -1){ dist[ele] = newDist; whoIsBack[ele]=nodeIndex;}
            else {if(newDist<dist[ele]){dist[ele] =newDist;whoIsBack[ele]=nodeIndex;}
            }
                }
            }
